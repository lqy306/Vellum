...
{
    AdwSwitchRow *source_row;
    AdwSwitchRow *script_row;
    AdwSwitchRow *web_row;
...
static void
ai_code_summary_config_widgets_free_internal(AiCodeSummaryConfigWidgets *widgets)
{
    g_free(widgets);
}
...
ai_code_summary_add_config_group(AdwPreferencesPage *page, GKeyFile *settings)
{
    ...
    adw_preferences_group_set_title(group, _("AI Code Summary"));
...

    {
        ...
        gtk_label_set_markup(GTK_LABEL(note),
                             g_strdup_printf(
                                 "%s\n\n"
                                 "<span foreground=\"shade12/1\" style=\"italic\">"
                                 "%s</span>",
                                 _("When you turn off automatic summarisation to save API costs, "
                                   "it is recommended to write a concise file-header comment at the top of each source file. "
                                   "Describe the file's purpose, its public API, major data structures, and key invariants. "
                                   "A good header lets readers (and AI tools) understand the file without tracing every line."),
    ...
    widgets->source_row = ADW_SWITCH_ROW(adw_switch_row_new());
    adw_preferences_row_set_title(ADW_PREFERENCES_ROW(widgets->source_row), _("Source files"));
    adw_action_row_set_subtitle(ADW_ACTION_ROW(widgets->source_row),
                                _("C, C++, Rust, Go, Java and other compiled-language source files."));
    adw_switch_row_set_active(widgets->source_row,
                              ai_code_summary_get_boolean(settings, "source", TRUE));
    adw_preferences_group_add(group, GTK_WIDGET(widgets->source_row));

    widgets->script_row = ADW_SWITCH_ROW(adw_switch_row_new());
    adw_preferences_row_set_title(ADW_PREFERENCES_ROW(widgets->script_row), _("Script files"));
    adw_action_row_set_subtitle(ADW_ACTION_ROW(widgets->script_row),
                                _("Python, Shell, Ruby, Perl, PHP and similar scripts."));
    adw_switch_row_set_active(widgets->script_row,
                              ai_code_summary_get_boolean(settings, "scripts", TRUE));
    adw_preferences_group_add(group, GTK_WIDGET(widgets->script_row));

    widgets->web_row = ADW_SWITCH_ROW(adw_switch_row_new());
    adw_preferences_row_set_title(ADW_PREFERENCES_ROW(widgets->web_row), _("Web files"));
    adw_action_row_set_subtitle(ADW_ACTION_ROW(widgets->web_row),
                                _("JavaScript, TypeScript, HTML, CSS and component files."));
    adw_switch_row_set_active(widgets->web_row,
                              ai_code_summary_get_boolean(settings, "web", TRUE));
    adw_preferences_group_add(group, GTK_WIDGET(widgets->web_row));

    widgets->data_row = ADW_SWITCH_ROW(adw_switch_row_new());
    adw_preferences_row_set_title(ADW_PREFERENCES_ROW(widgets->data_row), _("Structured data files"));
    adw_action_row_set_subtitle(ADW_ACTION_ROW(widgets->data_row),
                                _("JSON, YAML, TOML, XML and SQL files."));
    adw_switch_row_set_active(widgets->data_row,
                              ai_code_summary_get_boolean(settings, "data", FALSE));
    adw_preferences_group_add(group, GTK_WIDGET(widgets->data_row));
    ...
    gtk_adjustment_set_value(adw_spin_row_get_adjustment(widgets->interval_row),
                             ai_code_summary_get_uint(settings, "line-interval", AI_SUMMARY_DEFAULT_INTERVAL));
...
ai_code_summary_config_requires_cost_warning(AiCodeSummaryConfigWidgets *widgets)
{
    ...
    any_enabled =
        adw_switch_row_get_active(widgets->source_row) ||
        adw_switch_row_get_active(widgets->script_row) ||
        adw_switch_row_get_active(widgets->web_row) ||
        adw_switch_row_get_active(widgets->data_row);
    if (!any_enabled)
    {
        return FALSE;
    }
...
ai_code_summary_config_save(GKeyFile *settings, AiCodeSummaryConfigWidgets *widgets)
{
    ...
    g_key_file_set_boolean(settings, AI_SUMMARY_GROUP, "source",
                           adw_switch_row_get_active(widgets->source_row));
    g_key_file_set_boolean(settings, AI_SUMMARY_GROUP, "scripts",
                           adw_switch_row_get_active(widgets->script_row));
    g_key_file_set_boolean(settings, AI_SUMMARY_GROUP, "web",
                           adw_switch_row_get_active(widgets->web_row));
    g_key_file_set_boolean(settings, AI_SUMMARY_GROUP, "data",
                           adw_switch_row_get_active(widgets->data_row));
