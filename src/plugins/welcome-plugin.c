...
welcome_update_navigation(MtWelcomeData *data)
{
    ...
    progress = g_strdup_printf(welcome_text("关卡 %u / 3", "Level %u / 3"), data->page + 1);
    ...
    gtk_button_set_label(data->next_button,
                         data->page == 2 ? welcome_text("完成引导", "Finish guide") :
                         welcome_text("下一关", "Next level"));
    if (data->page == 1)
    {
        gtk_widget_set_sensitive(GTK_WIDGET(data->next_button), data->practice_complete);
    }
    else
    {
...
static void
welcome_practice_changed(GObject *object, GParamSpec *pspec, gpointer user_data)
{
    MtWelcomeData *data;
    gchar *text;
    ...
    (void)pspec;
    data = user_data;
    text = g_strdup(gtk_editable_get_text(GTK_EDITABLE(object)));
    data->practice_complete = *g_strstrip(text) != '\0';
    g_free(text);
    welcome_update_navigation(data);
}
...
welcome_build_intro_page(void)
{
    ...
    description = gtk_label_new(welcome_text("欢迎来到 Vellum。接下来用三个简短关卡完成首次编辑、选择扩展，并建立自己的工作流。",
                                              "Welcome to Vellum. Complete three short levels to make a first edit, choose extensions and establish your workflow."));
...
static GtkWidget *
welcome_build_practice_page(MtWelcomeData *data)
{
    GtkWidget *box;
    GtkWidget *title;
    GtkWidget *description;
    ...
    title = gtk_label_new(welcome_text("第一关：写下第一行", "Level 1: write your first line"));
    gtk_widget_add_css_class(title, "title-2");
    gtk_label_set_xalign(GTK_LABEL(title), 0.0);
    gtk_box_append(GTK_BOX(box), title);
    description = gtk_label_new(welcome_text("点击下方红框区域内的任意位置开始输入。输入任意内容后，即可解锁下一关。",
                                              "Click anywhere inside the red frame below to begin typing. Enter any text to unlock the next level."));
    gtk_label_set_wrap(GTK_LABEL(description), TRUE);
    gtk_label_set_xalign(GTK_LABEL(description), 0.0);
    gtk_box_append(GTK_BOX(box), description);
    frame = gtk_frame_new(NULL);
    gtk_widget_add_css_class(frame, "vellum-onboarding-target");
    gtk_widget_set_vexpand(frame, TRUE);
    gtk_widget_set_size_request(frame, -1, 170);
    view = gtk_entry_new();
    ...
    gtk_widget_set_margin_top(view, 12);
    gtk_widget_set_margin_bottom(view, 12);
    gtk_frame_set_child(GTK_FRAME(frame), view);
    gtk_box_append(GTK_BOX(box), frame);
    complete = gtk_label_new(welcome_text("完成条件：输入任意文字", "Goal: enter any text"));
    gtk_label_set_xalign(GTK_LABEL(complete), 0.0);
    gtk_widget_add_css_class(complete, "caption");
    gtk_box_append(GTK_BOX(box), complete);
    data->practice_input = GTK_EDITABLE(view);
    g_signal_connect(view, "notify::text", G_CALLBACK(welcome_practice_changed), data);
...
welcome_show_guide(MtWelcomeData *data)
{
    ...
    gtk_stack_add_named(data->pages, welcome_build_practice_page(data), "page-1");
    gtk_stack_add_named(data->pages, welcome_build_extensions_page(data), "page-2");
