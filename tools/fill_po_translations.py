...
ZH = {
    ...
    "Clear Output": "清除输出",
    "Hide Dev Experience Output": "隐藏开发体验输出",
    "Commands use direct argument parsing, not a shell. Placeholders: ${file}, ${dir}, ${name}, ${root}, ${profile}.\n": "命令使用直接参数解析而非 shell。占位符：${file}、${dir}、${name}、${root}、${profile}。\n",
    "Dev Experience Settings": "开发体验设置",
    "Explicit Commands": "显式命令",
    "Commands run only after F9, F10 or F11. They are parsed into arguments without a shell. Use ${file}, ${dir}, ${name}, or ${root}; ${name} is the source file name without its extension. Quote placeholders when paths may contain spaces.": "命令只会在按下 F9、F10 或 F11 后执行。命令会被解析为参数而不经过 shell。使用 ${file}、${dir}、${name} 或 ${root}；${name} 是去掉扩展名的源文件名。当路径可能包含空格时请给占位符加引号。",
    "Working Directory (optional)": "工作目录（可选）",
    "Build Command": "构建命令",
    "Run Command": "运行命令",
    "Build Profile (debug/release)": "构建配置（调试/发布）",
    "Debugger (e.g. gdb)": "调试器（如 gdb）",
    ...
    "Save Dev Experience Settings": "保存开发体验设置",
    ...
    "Build, run and debug with compiler error underlines and breakpoints": "在独立工具窗口构建、运行和调试，带编译器报错下划线和断点",
