...
static MtPluginActionCallback link_check_callback;
static gpointer link_check_data;
                ...
                GDestroyNotify destroy_notify)
{
    ...
    link_check_callback = callback;
    link_check_data = user_data;
...
test_get_current_text(MtPluginHost *host)
{
    ...
    return g_strdup("Good: http://127.0.0.1:18081/ok. Bad: http://127.0.0.1:1/unreachable");
}
...
test_show_toast(MtPluginHost *host, const gchar *message)
{
    ...
    if (g_strcmp0(message, "Link test complete: 1 reachable, 1 failed") == 0)
    {
        success = TRUE;
...
main(void)
{
    ...
    if (g_getenv("VELLUM_LINK_TEST_PLUGIN") == NULL)
    {
        return 2;
    }

    module = g_module_open(g_getenv("VELLUM_LINK_TEST_PLUGIN"), G_MODULE_BIND_LAZY);
    if (module == NULL)
    {
    ...
    if (!activate(&host, &error) || link_check_callback == NULL)
    {
    ...
    link_check_callback(NULL, NULL, link_check_data);
