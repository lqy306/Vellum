...
| 插件 | 基于 `GModule` 的动态扩展、持久化启停、导入/导出、配置与版本协商 | 所有内置扩展以源码 `.vut` 包分发；附带时间戳、统计、AI、链接测试、项目侧边栏、构建运行与 Vi Mode。首选项可关闭全部扩展，重启后只运行核心编辑器。 |
...
主菜单的“扩展”页面会列出模块名称、说明、版本与启用开关。禁用会移除扩展的动作、快捷键和面板，但不删除其文件；启用状态保存于用户配置目录。导出、配置和删除使用带工具提示的紧凑图标操作。用户导入模块可用垃圾桶按钮删除；随 AppImage 分发的内置模块可安全禁用但不能在运行中删除。关闭“启用扩展”总开关并重启后，扩展页会明确显示当前为核心模式且不允许导入或加载模块。模块名称和说明会在写入 Adwaita 偏好行前安全转义，因此 `Build & Run` 等包含 `&` 的名称不会再触发 markup 警告。原生源码扩展不会被沙箱隔离，必须只导入可信来源。
